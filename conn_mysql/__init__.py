...
class MySQL(object):
    ...
    def init_app(self, app):
        ...
        app.config.setdefault('MYSQL_PASSWORD', 'rootmysql')
        app.config.setdefault('MYSQL_DB', 'flaskapp')
        ...
        app.config.setdefault('USERNAME','zwx')
        app.config.setdefault('PASSWORD','zwx')
    ...
    def connection(self):
        ...
        if ctx is not None:
            if not hasattr(ctx, 'flask'):
                ctx.mysql_db = self.connect
            return ctx.mysql_db

    def teardown(self, exception):
        ctx = _app_ctx_stack.top
        if hasattr(ctx, 'flask'):
            ctx.mysql_db.close()